...
const Upload = () => {
    ...
    const handleAnalyze = async ({ companyName, jobTitle, jobDescription, file }: { companyName: string, jobTitle: string, jobDescription: string, file: File  }) => {
        ...
        if (file.type && file.type !== 'application/pdf') {
            setIsProcessing(false);
            return setStatusText('Error: Please upload a PDF file (.pdf)');
        }
        ...
        try {
            ...
            setIsProcessing(false);
            const msg = err?.status === 401 || String(err?.message || err).includes('401')
                ? 'You are not signed in. Please sign in and try again.'
                : 'Failed to upload file';
            return setStatusText(`Error: ${msg}`);
        }
        if(!uploadedFile) {
            setIsProcessing(false);
        ...
        if(!imageResult.file) {
            ...
            setIsProcessing(false);
            return setStatusText(`Error: ${imageResult.error || 'Failed to convert PDF to image'}`);
        }
